...
func (i IndexPlayListMovieInMyPage)Find(userId uint64,playListId uint64)*queryService.IndexPlayListMovieInMyPageDTO{
	...
	db.Raw("select movies.id as movie_id,movies.description as movie_title,movies.description as movie_description, movies.thumbnail_name as movie_thumbnail_name,play_list_movies.order as `order` " +
