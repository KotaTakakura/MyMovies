...
func (i IndexPlayListsInMyPage) All(userId uint64) *queryService.IndexPlayListsInMyPageDTO {
	...
	searchResult := db.Raw(
		"select play_lists.id as play_list_id, play_lists.name as play_list_name,play_lists.description as play_list_description,s.movie_id as play_list_first_movie_id,movies.thumbnail_name as play_list_first_movie_thumbnail_name "+
			"from play_lists "+
